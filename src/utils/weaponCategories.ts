...
const RULES: Array<[WeaponCategory, RegExp]> = [
  ...
  ['Naval Systems', /ship|vessel|frigate|destroyer|submarine|littoral|coast.*guard|patrol.*boat|patrol craft|naval|maritime|sonar|torpedo.*launch|anti-ship|aegis|phalanx|CIWS|cutter|surface combatant|vertical launching|VLS|Archangel|boat|mk\s*75|76\s*mm\s*gun|gun\s*mount/i],
  // \b before C-\d+ so ARC-210 radios aren't classified as C-130-style aircraft
  ['Aircraft', /helicopt|aircraft|F-\d+|AH-\d+|UH-\d+|CH-\d+|MH-\d+|P-\d+|\bC-\d+|KC-\d+|V-\d+|MQ-\d+|RQ-\d+|drone|UAV|UAS|UAS\b|Reaper|Predator|AWACS|Hornet|fixed.wing|rotary|airframe|turbine|engine|propeller|simulator.*flight|flight.*sim|aerostat|LAIRCM|ATFLIR|helmet\s*mounted\s*cueing|\bjhmcs\b/i],
  ['Missiles & Munitions', /missile|munition|rocket|bomb|JDAM|AMRAAM|AIM-|AGM-|GBU-|CBU-|JSOW|MK-\d+.*bomb|Javelin|Stinger|HIMARS|MLRS|APKWS|THAAD|Patriot|Tomahawk|Harpoon|Excalibur|projectile|IBCS|Integrated Battle Command|air.?defense|fire.?control|precision kill weapon|air.?strike|strike weapon|torpedo|warhead|guidance|fuze|propellant|ammunition|ammo|round|cartridge|grenade|mortar|ordnance|Volcano|ALTIUS|loitering|counter.?UAS|counter.?unmanned|ITAS|target acquisition|precision\s*guided\s*kit|\bm1156\b|\bpgk\b/i],
  ...
  ['Electronics & Communications', /radar|sensor|radio|communication|satellite|command.*control|C2|C4I|electronic|cyber|network|software|data.*link|LINK.?16|MIDS|Multifunctional Information Distribution|navigation|GPS|IFF|transponder|countermeasure|EW |electronic.*warfare|infrared.*system|targeting.*pod|surveillance|ISR|reconnaissance|recce|night.?vision|traffic.?control|landing.?system|terminal|oceanographic|repeater|border.?security|Mobile User Objective|high\s*gain\s*measurement|\bhgms\b|line\s*array|acoustic/i],
