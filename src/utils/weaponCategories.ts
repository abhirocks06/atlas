...
const RULES: Array<[WeaponCategory, RegExp]> = [
  ...
  ['Aircraft', /helicopt|aircraft|F-\d+|AH-\d+|UH-\d+|CH-\d+|MH-\d+|P-\d+|\bC-\d+|KC-\d+|V-\d+|MQ-\d+|RQ-\d+|drone|UAV|UAS|UAS\b|Reaper|Predator|AWACS|Hornet|fixed.wing|rotary|airframe|turbine|engine|propeller|simulator.*flight|flight.*sim|aerostat|LAIRCM|ATFLIR/i],
